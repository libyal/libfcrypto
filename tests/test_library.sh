...
LIBRARY_TESTS="error rc4_context serpent_context support";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./fcrypto_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfcrypto" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
